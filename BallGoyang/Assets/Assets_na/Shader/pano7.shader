				...
				struct Input {
					...
					float2 uv_MainTex11;
						...
						void surf(Input IN, inout SurfaceOutputStandard o) {
							...
							if (_playerPos >= 90 && _playerPos < 95)
							{
								fixed4 c = tex2D(_MainTex19, IN.uv_MainTex19);
								fixed4 d = tex2D(_MainTex20, IN.uv_MainTex20);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							}

							else if (_playerPos >= 95 && _playerPos < 100)
							{
								fixed4 c = tex2D(_MainTex20, IN.uv_MainTex20);
								fixed4 d = tex2D(_MainTex21, IN.uv_MainTex21);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							}

							//else if (_playerPos >= 100 && _playerPos < 105)
							//{
