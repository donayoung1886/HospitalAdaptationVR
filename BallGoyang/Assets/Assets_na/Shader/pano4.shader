				...
				struct Input {
					...
					/*		float2 uv_MainTex14;
						...
						void surf(Input IN, inout SurfaceOutputStandard o) {
							...
							if (_playerPos >= 45 && _playerPos < 50)
							{
								fixed4 c = tex2D(_MainTex10, IN.uv_MainTex10);
								fixed4 d = tex2D(_MainTex11, IN.uv_MainTex11);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							}

							else if (_playerPos >= 50 && _playerPos < 55)
							{
								fixed4 c = tex2D(_MainTex11, IN.uv_MainTex11);
								fixed4 d = tex2D(_MainTex12, IN.uv_MainTex12);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							}

							else if (_playerPos >= 55 && _playerPos < 60)
							{
								fixed4 c = tex2D(_MainTex12, IN.uv_MainTex12);
								fixed4 d = tex2D(_MainTex13, IN.uv_MainTex13);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							}
