					 ...
					 struct Input {
						 ...
						 //   float2 uv_MainTex12;
						  ...
						  void surf(Input IN, inout SurfaceOutputStandard o) {
							 ...
							 if (_playerPos >= 60 && _playerPos < 65)
							 {
								fixed4 c = tex2D(_MainTex13, IN.uv_MainTex13);
								fixed4 d = tex2D(_MainTex14, IN.uv_MainTex14);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							 }

							 else if (_playerPos >= 65 && _playerPos < 70)
							 {
								fixed4 c = tex2D(_MainTex14, IN.uv_MainTex14);
								fixed4 d = tex2D(_MainTex15, IN.uv_MainTex15);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							 }

							 else if (_playerPos >= 70 && _playerPos < 75)
							 {
								fixed4 c = tex2D(_MainTex15, IN.uv_MainTex15);
								fixed4 d = tex2D(_MainTex16, IN.uv_MainTex16);
								o.Albedo = lerp(c.rgb, d.rgb, _lerp);
								//o.Alpha = c.a;
							 }
